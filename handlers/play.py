...
async def play(_, message: Message):
    global que
    lel = await message.reply("🔄 **Processing Via ✯AnushkaMusic✯**")
    ...
    await lel.edit("🔎 **Finding Via ✯AnushkaMusic✯**")
    sender_id = message.from_user.id
    ...
    for i in message.command[1:]:
        query += ' ' + str(i)
    print(query)
    await lel.edit("🎵 **Processing Via ✯AnushkaMusic✯**")
            ...
            [   
                ...
                [
                   InlineKeyboardButton(
                       text="⭐ Support ⭐",
                       url='https://t.me/anabelle_secret')
                ],
                [       
                    InlineKeyboardButton(
                        text="❌ Close",
    ...
    if message.chat.id in callsmusic.pytgcalls.active_calls:
        ...
        photo="final.png",
        reply_markup=keyboard,
        caption="▶️ **Playing** song requested by {} via AnushkaMusic ".format(
...
async def deezer(client: Client, message_: Message):
    global que
    lel = await message_.reply("🔄 **Processing Via ✯AnushkaMusic✯**")
    administrators = await get_administrators(message_.chat)
    chid = message_.chat.id
    try:
        user = await USER.get_me()
    except:
        user.first_name =  "AnushkaMusic"
    usar = user
    wew = usar.id
    try:
        #chatdetails = await USER.get_chat(chid)
           ...
           for administrator in administrators:
                      if administrator == message_.from_user.id:  
                          ...
                          try:
                              ...
                              await lel.edit(
                                  f"<b>🔴 Flood Wait Error 🔴 \nUser {user.first_name} couldn't join your group due to heavy requests for userbot! Make sure user is not banned in group."
                                  "\n\nOr manually add @anabelle_secret_bot to your Group and try again</b>",
                              )
                              pass
    try:
        chatdetails = await USER.get_chat(chid)
    ...
    if message_.chat.id in callsmusic.pytgcalls.active_calls:
        ...
        appendable = [s_name, r_by, loc]
        qeue.append(appendable)
        await res.edit_text(f"✯AnushkaMusic✯= #️⃣ Queued at position {position}")
    else:
        await res.edit_text("✯AnushkaMusic✯=▶️ Playing.....")
...
async def jiosaavn(client: Client, message_: Message):
    ...
    if message_.chat.id in callsmusic.pytgcalls.active_calls:
        ...
        m = await client.send_photo(
            ...
            reply_markup=keyboard,
            photo="final.png",
            caption=f"✯AnushkaMusic✯=#️⃣ Queued at position {position}",
